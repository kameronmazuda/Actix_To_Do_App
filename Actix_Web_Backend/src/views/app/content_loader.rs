...
pub fn read_file(file_path: &str) -> String {
	let contents: String = fs::read_to_string(file_path).expect("Failed to read file.");
...
pub fn add_component(component_tag: String, html_data: String) -> String {
	...
	let css_path = String::from("./templates/components/".to_string() + &component_tag.to_lowercase() + ".css");
	let css_loaded = read_file(&css_path);
	
	let html_path = String::from("./templates/components/".to_string() + &component_tag.to_lowercase() + ".html");
